...
function baseSecao(tipo: SecaoTipo, dados: Record<string, any> = {}): SiteSecao {
  return {
    id: "preview", tipo,
    eyebrow: "Preview", titulo: labelForTipo(tipo),
    descricao: "Descrição de exemplo para validar responsividade em telas mobile pequenas com texto suficiente para caber em duas linhas.",
    descricao_extra: null, imagem_url: null,
    cta_texto: "Falar no WhatsApp", cta_link: "https://wa.me/5511932139815",
    bg_style: "branco", bg_cor: null, bg_cor_2: null,
    order: 0, enabled: true,
    texto_cor: null, card_bg_cor: null, card_texto_cor: null, card_borda_cor: null,
    itens: [], dados,
  };
}
  ...
  switch (tipo) {
    case "cta-banner": {
      ...
      if (layout === "com-imagem") {
        s.imagem_url = "https://images.unsplash.com/photo-1526413232644-8a40f03cc03b?auto=format&fit=crop&w=1600&q=60";
      }
      return s;
    }
    case "equipe":
...
function DevPreview() {
  ...
  if (!tipo) return <div className="p-8 text-sm">Use ?tipo=cta-banner&amp;layout=centralizado</div>;
  const secao = buildSecao(tipo as SecaoTipo, layout);
  return (
    <div data-dev-preview={`${tipo}:${layout ?? "default"}`}>
      <DynamicSection secao={secao} team={MOCK_TEAM} testimonials={MOCK_TESTIMONIALS} servicos={MOCK_SERVICOS} />
    </div>
  );
}