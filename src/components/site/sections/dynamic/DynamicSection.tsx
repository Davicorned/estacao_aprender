    ...
    <section className={`${bg} py-20`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
          ...
          <FadeUp delay={0.15} className={secao.imagem_url ? "" : "lg:col-span-2"}>
            <Eyebrow text={secao.eyebrow} />
            {secao.titulo && (
              <h2 className="mt-3 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
                {secao.titulo}
            ...
            {secao.itens.length > 0 && (
              <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
                {secao.itens.map((it) => {
                  ...
                  return (
                    <div key={it.id} className="flex items-start gap-3 rounded-xl bg-[#FEF3E8] p-3">
                      <Icon className="h-5 w-5 shrink-0 text-[#D67F43] mt-0.5" />
                      <div>
                        <p className="text-sm font-medium text-gray-700">{it.titulo}</p>
                        {it.descricao && (
                          <p className="text-xs text-gray-500 mt-0.5">{it.descricao}</p>
                        )}
                      </div>
                    </div>
                  );
