...
export function EquipeManager() {
  ...
  async function move(id: string, dir: -1 | 1) {
    ...
    const a = items[idx];
    const b = items[swap];
    const r1 = await supabase.from("team_members").update({ order: b.order }).eq("id", a.id);
    const r2 = await supabase.from("team_members").update({ order: a.order }).eq("id", b.id);
          ...
          {items.map((m, idx) => (
            ...
            >
              ...
              <div className="flex items-center gap-1">
                <Button size="icon" variant="ghost" onClick={() => move(m.id, -1)} disabled={idx === 0}>
