...
import { LayoutDashboard, Calendar, Users, FileText, DollarSign, UserCog, MessageSquareQuote, ExternalLink, LogOut, Menu, Settings, Loader2, Sun, Moon, Monitor, Sparkles, Palette, Briefcase, Stethoscope, Globe, MessageSquare, Newspaper, MessageCircle, Bot, Inbox, } from "lucide-react";
...
const GestaoTitleContext = createContext(null);
export function useGestaoTitle(title) {
...
function NavLinks({ onNavigate }) {
    ...
    const renderItem = (item) => {
        ...
        return (<Link key={item.to} to={item.to} onClick={onNavigate} className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors ${active
    ...
    const renderSoon = (item) => {
        const Icon = item.icon;
        return (<div key={item.label} className="flex items-center justify-between rounded-lg px-3 py-2 text-sm text-gray-400 dark:text-muted-foreground cursor-not-allowed" title="Em breve">
...
function ThemeToggle() {
    ...
    return (<DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button type="button" aria-label="Alternar tema" className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-muted-foreground dark:hover:bg-accent dark:hover:text-foreground">
          <Icon className="h-4 w-4"/>
...
function SidebarFooter({ onSignOut }) {
    const { user } = useAuth();
    return (<div className="space-y-1 border-t border-gray-200 p-3 dark:border-border">
      <Link to="/gestao/configuracoes" className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-muted-foreground dark:hover:bg-accent dark:hover:text-foreground">
        <Settings className="h-4 w-4"/>
        Configurações
      </Link>
      <ThemeToggle />
      <a href="/" target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-muted-foreground dark:hover:bg-accent dark:hover:text-foreground">
        <ExternalLink className="h-4 w-4"/>
        Ver site
      </a>
      <button type="button" onClick={onSignOut} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-muted-foreground dark:hover:bg-accent dark:hover:text-foreground">
        <LogOut className="h-4 w-4"/>
        Sair
      </button>
      {user?.email && (<p className="px-3 pt-2 text-[11px] text-gray-400 truncate dark:text-muted-foreground">{user.email}</p>)}
    </div>);
}
export function GestaoShell({ title, children }) {
    ...
    const [temaLogo, setTemaLogo] = useState(null);
    useEffect(() => {
        let alive = true;
        void fetchTema().then((t) => { if (alive)
            setTemaLogo(t?.logo_url ?? null); });
        return () => { alive = false; };
    }, []);
    ...
    return (<GestaoTitleContext.Provider value={ctxValue}>
    <div className="flex min-h-screen bg-gray-50 dark:bg-background">
      ...
      <main className="relative flex-1 overflow-x-clip">
        {/* Loading bar */}
        {isPending && (<div className="fixed top-0 left-0 right-0 z-50 h-1 overflow-hidden bg-[#D67F43]/20 md:left-64">
        ...
        <div className="relative p-6">
          {showLoading && (<div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60 backdrop-blur-[1px] dark:bg-background/60">
